...
export class ProjectRepository implements IProjectRepository {
  ...
  async getFeatured(): Promise<ProjectEntity[]> {
    return PROJECTS_DATA.filter((p) => p.featured).map((p) => this.toEntity(p));
  }

  async getByCategory(category: string): Promise<ProjectEntity[]> {
    return PROJECTS_DATA.filter((p) => p.category === category).map((p) =>
      this.toEntity(p),
    );
  }
  ...
  async getRecent(limit: number = 6): Promise<ProjectEntity[]> {
    ...
    return projects.filter((p) => p.isRecent).slice(0, limit);
  }
